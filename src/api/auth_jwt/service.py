...
class JwtTokenService:
    ...
    async def create_tokens(self, user_uuid: UUID, response: Response, agree_status: bool):
        ...
        await self.create_cookie(jwt_refresh_token=jwt_refresh_token, 
                                response=response, agree_status=agree_status)
        return {
    ...
    async def create_cookie(self, jwt_refresh_token: str, response: Response, agree_status: bool) -> None:
        self.config.set_refresh_cookies(jwt_refresh_token, 
                                    response, 
                                    max_age=1728000 if agree_status is True else None)
        


async def get_uuid_from_cookie(request: Request) -> str:
        try:
